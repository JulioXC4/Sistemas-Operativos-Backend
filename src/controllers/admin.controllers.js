...
const getUserRoleByIdAuth = async (req, res ) =>{
 ...
 */
    // Simulación de base de datos de usuarios y roles
    const userRoles = {
      'user123': 'admin',
      'user456': 'editor',
      'user789': 'viewer',
 ...
 /*    if (!role) {
      return res.status(404).send('El usuario no posee ningún rol o no fue encontrado');
    } */

    return res.send(userRoles.user123);
  } catch (error) {
    return res.status(500).send('Error del servidor');
  }
};
